...
const AdminUsers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      addToast(err.response?.data?.message || 'Failed to fetch admin users', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingUser) {
        ...
        addToast('Staff Admin created successfully!', 'success');
      }
  ...
  const handleResetPasswordSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      await resetStaffAdminPasswordApi(resetPasswordUser._id, newPassword);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast('Admin account deleted permanently!', 'success');
  ...
  return (
    <div>
      <AdminHeader setMobileOpen={setMobileOpen} title="Staff Admin User Management" />
      ...
      <main className="p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          <div>
            <h2 className="text-xl font-bold font-serif text-slate-900 flex items-center gap-2">
              <FiUsers className="text-blue-600" /> Manage Staff Admins
            </h2>
            <p className="text-xs text-slate-500 mt-1">
              Create, edit, reset passwords, or manage access permissions for school staff accounts.
            </p>
          </div>
          ...
          >
            <FiUserPlus className="text-lg" /> Create Staff Admin
          </button>
        </div>
        ...
        {loading ? (
          <div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200 text-center">
            ...
            <p className="text-sm font-semibold text-slate-600">Loading admin user accounts...</p>
          </div>
        ) : users.length > 0 ? (
                    ...
                    <tr key={u._id} className="hover:bg-slate-50/80 transition-colors">
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center text-base font-bold border border-blue-100">
          ...
          <div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200 text-center max-w-md mx-auto space-y-3">
            ...
            <h3 className="text-lg font-bold font-serif text-slate-800">No Staff Admins Created</h3>
            <p className="text-xs text-slate-500">
              Create staff user accounts to give teachers or school staff restricted access to Gallery and Notices.
            </p>
          </div>
        )}
      </main>
          ...
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl overflow-hidden border border-slate-200">
            <div className="primary-gradient text-white p-6 flex justify-between items-center">
              <h3 className="text-lg font-bold font-serif flex items-center gap-2">
                <FiShield /> {editingUser ? 'Edit Staff Account' : 'Create Staff Admin Account'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                >
                  <option value="staff">Staff Admin (Limited Access: Gallery, Notices, Messages)</option>
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            ...
            <div className="text-center">
              <h3 className="text-lg font-bold font-serif text-slate-900">Reset Password</h3>
              <p className="text-xs text-slate-500">
      ...
      {deleteConfirmUser && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 text-center space-y-4 shadow-2xl border border-slate-200">
            ...
            <h3 className="text-lg font-bold font-serif text-slate-900">Delete Staff Admin?</h3>
